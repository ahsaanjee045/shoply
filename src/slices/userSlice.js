...
export const loginUser = createAsyncThunk("user/loginUser", async (user) => {
   try {
    let res = await axios.post(`${process.env.REACT_APP_API_URL}api/v1/user/login`, user)
    return res.data;
   } catch (error) {
    throw new Error(error.response.data.message);
   }
...
export const fetchAllUsers = createAsyncThunk("user/fetchAllUsers", async ({token}) => {
   try {
    let res = await axios.get(`${process.env.REACT_APP_API_URL}api/v1/user/allUser`, {
        headers : {
...
export const updateAdmin = createAsyncThunk("user/updateAdmin", async ({token, userId}) => {
   try {
    let res = await axios.put(`${process.env.REACT_APP_API_URL}api/v1/user/updateAdmin/${userId}`,{}, {
        headers : {
            authorization : `Bearer ${token}`
        }
    })
...
export const deleteUser = createAsyncThunk("user/deleteUser", async ({token, userId}) => {
    try {
     let res = await axios.delete(`${process.env.REACT_APP_API_URL}api/v1/user/deleteUser/${userId}`, {
         headers : {
             authorization : `Bearer ${token}`
         }
     })
